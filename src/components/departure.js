...
export default class Departure extends React.Component {
    ...
    render() {
        ...
        const modeIcon = this.props.detailed ? {height: '23%', width:'10%', minHeight:this.props.rowHeight * 0.3, minWidth:this.props.rowHeight * 0.1} :
            {marginTop: 7, height: '45%', width:'10%', minHeight:this.props.rowHeight * 0.5, minWidth:this.props.rowHeight * 0.1}
                ...
                <TouchableHighlight style={{width: '100%', height:'100%'}} onPress={() => this.props.parent.viewOneDeparture(this.props.num)} underlayColor='white'>
                    <View>
                        <View style={this.props.detailed ? split : styles.dontsplit} >
                            {mode === 'bus' ? <Image resizeMode='cover' style={modeIcon} source={LoadPicture('bus')} /> :
                                mode === 'transilien' ? <Image resizeMode='cover' style={modeIcon} source={LoadPicture('transilien')} /> :
                                    mode === 'rer' ? <Image resizeMode='cover' style={modeIcon} source={LoadPicture('rer')} /> :
                                        mode === 'intercités' ? <Image resizeMode='cover' style={bigModeIcon} source={LoadPicture('intercites')} /> :
                                            mode === 'intercités de nuit' ? <Image resizeMode='cover' style={bigModeIcon} source={LoadPicture('intercites')} /> :
