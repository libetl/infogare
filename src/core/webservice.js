...
export default {
    ...
    nextDepartures: async ({long, lat}, {token, notify = () => {}, mode = token ? 'sncfApi' : 'raildar'} = {}) => {
        const preferedSource = sources[mode]
        ...
        const stationsAreas = await preferedSource.stationSearch(guessedStationCoords, {guessedStationName, token})
        ...
        const baseDepartures = removeDuplicates(sortByTime(await preferedSource.baseDepartures(stationsAreas, token)))
